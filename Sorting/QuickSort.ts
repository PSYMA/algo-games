export default class QuickSort {
    ...
    private arr: Array<Sorting.IProps> = [];
    private isAlgorithmRunning = new Array<boolean>(1);
    constructor(arr: Array<Sorting.IProps>, ctx: CanvasRenderingContext2D, isAlgorithmRunning: Array<boolean>) {
        this.arr = arr;
        this.ctx = ctx;
    ...
    // recursion version
    private async Swap(items: Array<Sorting.IProps>, leftIndex: number, rightIndex: number) {
    ...
    private async Partition(items: Array<Sorting.IProps>, left: number, right: number) {
    ...
    private async QuickSort(items: Array<Sorting.IProps>, left: number, right: number) {
        if (items.length > 1) {
